...
#   filter(across(c(orcamento, receita), ~!is.na(.))) 
#   as.data.frame(dolar)
#  dolar_final<- gsub("\\$", "", dolar$orcamento)
#  dolar_final
#  dolar_dataframe <- as.data.frame(dolar_final)#|>
  
  # filter (gsub('[$]([0-9]+)[,]([0-9]+)','\\1\\2', dolar$orcamento))
 # filter(orcamento == starts_with("$"), receita == starts_with("$"))
  ## mutate(across(c(orcamento, receita),
  #               .fns = parse_number)
  # )
  ...
  #   .cols = c(orcamento, receita),
  #   .fns = ~ str_remove((.x), pattern = '[0-9]+')))) |> 
  #filter(across(c(orcamento, receita)), "\\$")
 # mutate(dolar_orc = as.character(dolar$orcamento, gsub("\\$", "Dolar", dolar$orcamento)),
 #       dolar_rec =  as.character(dolar$receita, gsub("\\$", "Dolar", dolar$receita))
   
dolar

# gender_movie_gain <- currency <- imdb |> 
#   filter(across(c(orcamento, receita), ~!is.na(.)))  |> 
#   summarise((across(
#     .cols = c(orcamento, receita),
#     .fns = ~ str_detect((.x), pattern = '$'))))
#                         
# gender_movie_gain
```
...
```{r}
nota <- imdb |>  
  filter(across(c(nota_imdb, orcamento,receita), ~!is.na(.)))  |>
  arrange(desc(nota_imdb)) |> 
  #select('Grade' = nota_imdb)
  #filter(str_detect(id_filme, pattern = "tt1305806$"))
nota
```
